...
void create_room()
{
    ...
    add_item( ({ "dial", "dials",}), "@@show_dials");
...
public string show_dials()
{
    return "The LENGTH dial is set to " + pos[0][length] + ".\n" +
...
public void turn_cmd(string str)
{
    string knob, set;
    int i;
    ...
    say(QCTNAME(this_interactive()) + " sets the " + knob + " to " + set + ".\n");
}
...
public int all_cmd(string str)
{
    switch(query_verb())
    {
		...
		default:
			if (({int}) this_interactive()->query_wiz_level() > 40)
				return 0;
...
public void mangle_player(object pl, int l, int w)
{
    ...
    if (!(({int}) pl->query_ghost() & GP_MANGLE))
		return;
    ...
    attr = RACEATTR[({string}) pl->query_race_name()];

    if (!sizeof(attr))
    {
		write("You have a bad body, go get a better one.\n");
		pl->set_ghost(({int}) pl->query_ghost() | GP_BODY);
		pl->ghost_start();
    }
    ...
    write("you come out of it as a new " + ({string}) pl->query_race_name() + ".\n\n");

    pl->set_ghost(({int}) pl->query_ghost() & ~GP_MANGLE);

    if (({int}) pl->query_ghost() & GP_FEATURES)
		pl->move_living("X", PATH + "features");
    else if (({int}) pl->query_ghost() & GP_SKILLS)
