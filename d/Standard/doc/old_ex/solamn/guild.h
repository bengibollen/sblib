...
#define SKILLS ({ 20, 21, 23, 24,  0, 3, 4, SS_RESCUE, SS_BASH, SS_KICK })

/* Formulas to the cmd_soul */
#define F_CKICK_HIT(ob, ob2)	ob->query_skill(SS_KICK) * ob->query_stat(SS_DEX)

#define F_CKICK_PEN(ob, ob2)	F_PENMOD(ob->query_stat(SS_STR) / 4, ob->query_skill(SS_KICK) / 2) + 10

#define CKICK_HIT				1000

#define F_BASH_HIT(ob, ob2)		ob->query_stat(SS_DIS) * ob->query_skill(SS_BASH)

#define F_BASH_PEN(ob, ob2)		F_PENMOD(ob->query_stat(SS_STR) / 2, 2 * ob->query_skill(SS_BASH) / 3) + 20

#define BASH_HIT				1500

#define F_RESCUE(ob, ob2)		ob->query_stat(SS_DIS) * ob->query_stat(SS_DEX) * ob->query_skill(SS_RESCUE)
