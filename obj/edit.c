...
edit(mixed ffun, string str = "", int begin = 0)
{
    ...
    else if (!stringp(ffun) || !sizeof(ffun))
    {
...
add_cc(string arg)
{
    ...
    if (sizeof(arg = calling_ob->check_mailing_list(names)))
    {
        write("The name " + arg +
...
import(string arg)
{
     ...
     */
    if (member(EDIT_END, import_lines) != -1)
    {
        lines -= ({ EDIT_END });
...
autowrap(string str)
{
    str = (sizeof(str) ? lower_case(str) : "y");
    switch(str[0])
    {
    case '~':
    case 'q':
...
input(string str)
{
    ...
    if (str == EDIT_END)
    {
        ...
        if (sizeof(filter(lines, &operator(>=)(, 80) @ sizeof)))
        {
