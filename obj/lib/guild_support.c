...
get_prefs(string str)
{
    ...
    if ((index = member(words[0], SD_LONG_STAT_DESC)) == -1)
    {
        index = member(words[0], SD_STAT_DESC);
    }
    ...
    if ((index = member(words[1], SD_LONG_STAT_DESC)) == -1)
    {
        index = member(words[1], SD_STAT_DESC);
    }
    if ((index < 0) || (index >= SS_NO_EXP_STATS))
    {
        write("Unknown stat " + words[0] + ". Use either " +
...
gs_list(string str)
{
    ...
    {
        if (!sizeof(long_name = call_other(this_player(), "query_guild_name_" +
