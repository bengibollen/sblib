...
static string  book_dir  = "",
               ...
               appr_list = "",
               appr_list_short = "",
               text_object_name = DEFAULT_TEXT_FILE_NAME,
              *book_types = ({ "book" }),
...
library_borrow(string str)
{
    ...
    if (!sizeof((book_file = book_map[str])))
    {
        if (sscanf(str, "approval %s", str))
        {
            ...
            if (!sizeof(book_file = appr_map[str]))
            {
...
library_list(string str)
{
    ...
    if (sizeof(str) && sscanf(str, "%s titles", shelf) && book_shelves[shelf])
    {
    ...
    if (sizeof(appr_dir) && (str == "approval"))
    {
        if (!library_approve_access() || !library_admin_access())
        {
            return 1;
    ...
    if (sizeof(appr_dir) && (str == "approval titles"))
    {
        if (!library_approve_access() || !library_admin_access())
        {
            return 1;
...
get_book_name(string dir)
{
    ...
    {
        if (!sscanf(files[i], text_object_name + "%d", tmp))
        {
           continue;
        }
         ...
         string text, int approval)
{
    ...
    if (!approval && sizeof(default_shelf))
    {
        dir += default_shelf + "/";
    }
...
library_classify_get_shelf(object who, string title, string input)
{
    ...
    if (!sizeof(book_file = book_map[title]))
    {
