...
init_map()
{
	...
	if (sizeof(line) && line[0..0] != "\n" && line[0..0] != "#")
	{
	    ...
	    Map_w = Map_w > sizeof(line) ? Map_w : sizeof(line);
	    Map_h++;
	}
	else if (sizeof(line) >= 6 && line[0..5] == "#NAME ")
	    Mapname = explode(line[6..sizeof(line)], "\n")[0];
	else if (sizeof(line) >= 9 && line[0..8] == "#ROOMDIR ")
	    Roomdir = explode(line[9..sizeof(line)], "\n")[0];
	else if (sizeof(line) >= 9 && line[0..8] == "#AREADIR ")
	...
	else if (sizeof(line) >= 9 && line[0..8] == "#TIMEOUT ")
	{
	    ...
	    Trashcan = explode(line[10..sizeof(line)], "\n")[0];
	lno++;
    } while(sizeof(line));
    Map_w++; /* Newline */
...
enter_map(string entryp)
{
    ...
    if (!sizeof(room) || !create_room(room))
    {
...
create_room(string room)
{
	...
	if (sizeof(dir))
		...
		write_file(rmd, "    add_exit(\"" + dir[1..sizeof(dir)] + "\", \"" + dirs[i] + "\", 0, " + desc[0][3] + ");\n");
    }
...
{
    if (!sizeof(Trashcan) || catch(ob->move(Trashcan, 1)))
	ob->remove_object();
}
...
del_rooms()
{
	...
	if (rooms[i][0..(sizeof(Mapname) - 1)] == Mapname)
...
index(string str, string sstr)
{
    ...
    if (sizeof(brk) > 1)
	return sizeof(brk[0]);
    else if (str[0..(sizeof(sstr) - 1)] == sstr)
	return 0;
    else if (str[(sizeof(str) - sizeof(sstr))..(sizeof(str) - 1)] == sstr)
	return sizeof(str) - 1;
    else
	return -1;
}
