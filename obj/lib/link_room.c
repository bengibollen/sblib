...
transport_to(string ex, mixed room, int delay)
{
    ...
    /* Find or == objectp, room = object_name
	*/
    if (stringp(room))
    {
	or = find_object(room);
    ...
    {
	or = room;
	if (!sscanf(object_name(or), "%s#", room))
	    room = object_name(or);
    }
...
make_link(mixed to_room, object via_link)
{
    ...
    if (!(sscanf(object_name(to_room), "%s#", to_room)))
	to_room = object_name(to_room);

    if (sizeof(link_ends))
    {
	ne = member(to_room, link_ends);
	if (ne >= 0)
