...
static varargs string desc_live_dead(mixed arr, object for_obj, int include_no_show)
{
    return composite(arr, "short", #'desc_same, for_obj, include_no_show);
}


varargs string desc_live(mixed arr, int no_someone, object for_obj, int include_no_show)
{
    string str;
...
varargs string fo_desc_live(mixed arr, object for_obj, int no_someone, int include_no_show)
{
    if (!for_obj)
    {
...
varargs string fo_desc_last_live()
{
...
varargs string desc_dead(mixed arr, int no_something, object for_obj, int include_no_show)
{
    string str;
...
varargs string fo_desc_dead(mixed arr, object for_obj, int no_something, int include_no_show)
{
    if (!for_obj)
    {
        for_obj = previous_object(-1);
...
varargs string fo_desc_last_dead()
{
    object for_obj = previous_object(-1);
...
string desc_same(object *oblist, object for_obj)
{
    ...
    if (({int}) ob->query_prop(OBJ_I_HIDE) > 0)
    {
	pre = "[";
	aft = "]";
    }

    if (({int}) ob->query_prop(OBJ_I_INVIS) > 0)
    {
	pre = "(";
	aft = ")";
    }

    sshort = ({string}) ob->short(for_obj);
    ...
    else if (({int}) ob->query_prop(HEAP_I_IS))
    {
...
varargs string fo_composite(mixed arr, string sepfunc, closure descfunc, object for_obj)
{
    if (!for_obj)
    {
    ...
    int include_no_show)
{
    ...
    if (!include_no_show)
    {
        arr = filter(arr, (: !({int}) $1->query_no_show_composite() :));
    }

    OldArr = arr;

    arr = filter(arr, (: ({int}) $1->check_seen(for_obj) :));
 ...
 *
 */
mixed sort_similar(mixed arr, string sepfunc)
{
    int b, i;
    mixed *a;
 ...
 */
varargs string composite_words(string *wlist, string word = "and")
{
    if (!sizeof(wlist))
    {
    	return "";
    ...
    return implode(wlist[..<2], ", ") + " " + word + " " + wlist[<1];
}
 ...
 */
public string hanging_indent(string to_print, int length, int width)
{
    int    scrw;
    string *tmp;
    ...
    {
    case 0:
        scrw = ({int}) this_player()->query_option(OPT_SCREEN_WIDTH);
     ...
     */
    tmp = explode(to_print, "\n");
    return (implode( ({ tmp[0] }) +
        (explode(implode(tmp[1..], " "), "\n")),
        ("\n" + "                    "[1..length])) + "\n");
}
