 ...
 */
public nomask void create_skill_raise()
{
 ...
 */
public nomask string *sk_query_main_levels()
{
    return desc + ({ });
}
 ...
 * Returns      : string * - the array.
 */
public nomask string *sk_query_sub_levels()
{
    return subdesc + ({ });
}
 ...
 *                              weight * stat / 100
 */
public varargs void sk_add_train(
    mixed snum,
    mixed desc,
    string name,
    int costf,
 ...
 */
public int sk_do_train(int snum, object pl, int to_lev)
{
    mixed skval;
 ...
 */
    if ((({int}) this_player()->query_average_stat() <= 20) &&
        ((to - fr) <= 10 ))
    {
        cf /= 2;
    }
...
public string sk_rank(int lev)
{
    ...
    return (sizeof(subdesc[subl]) ? subdesc[subl] + " " : "") + desc[mainl];
}
 ...
 */
public string sk_query_name(int skill)
{
    if (!mappingp(sk_default))
    {
        return 0;
 ...
 */
public int sk_find_skill(string skname)
{
    mixed *skarr, *skix;
    int il;
 ...
 */
varargs string sk_fix_cost(int snum, int steps)
{
    int this_level, next_level, max;
    string next_rank, max_rank, cost;

    this_level = ({int}) this_player()->query_base_skill(snum);
...
void sk_hook_raise_rank(int snum, int to_lev, int cost)
{
    ...
    say(QCTNAME(this_player()) + " improves " +
        ({string}) this_player()->query_possessive() + " ability to " +
 ...
 */
void sk_hook_write_header(int steps)
{
    if (!steps)
    {
        steps = 1;
 ...
 */
void sk_hook_also_learn(int num)
{
    if (num == 1)
 ...
 */
public int sk_hook_not_allow_train()
{
 ...
 * Returns:       string -- fail message
 */
public varargs int sk_hook_not_allow_train_skill(string skill)
{
...
{
    return ((({int}) this_player()->query_base_skill(sk) == 0) &&
...
int sk_filter_improve(int sk, int steps)
{
    ...
    skill = ({int}) this_player()->query_base_skill(sk);
    return ((skill > 0) &&
...
int sk_improve(string str)
{
    ...
    known_sk = ({int *}) this_player()->query_all_skill_types();
    ...
    level = ({int}) this_player()->query_base_skill(snum);
