...
convert_to_names(string *list)
{
    ...
    foreach(string name: list)
    {
        ...
        if (is_wizard)
        {
            ...
            if ((type = member(LANG_SWORD(name), WIZ_N)) > -1)
            {
...
from(string str)
{
    ...
    while(++index < size)
    {
        ...
        if (sizeof(tmp = pMessages[index][MAIL_SUBJ]) > 38)
            tmp = tmp[0..34] + "...";
...
erase()
{
    ...
    while(--index >= 0)
    {
         ...
         */
        if (sizeof(message[MSG_ADDRESS]) == 0)
        {
