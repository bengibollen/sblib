...
slots_free()
{
    ...
    wizards = sizeof(filter(list, (: $1->query_wiz_level() :)));
...
enqueue(object ob)
{
     ...
     * very nice ;-)
     */
    if ((pos = member(ob->query_pl_name(), queue_list(1))) != -1)
    {
...
{
    return member(name, queue_list(1));
}
...
set_vip(string v)
{
    ...
    if ((pos = member(v, queue_list(1))) != -1)
    {
