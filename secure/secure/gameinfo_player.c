...
write_info()
{
    ...
    pos = member(cur_sect, sect_array);
    if (pos >= 0)
	...
	sizeof(sect_array[pos+1]) > sizeof(sect_array[pos]))
	cur_menu = fix_menu(cur_sect);
...
// This looks like dead code, I wonder what it was supposed to do /Tintin
	...
	if (sizeof(sect_array[pos+1]) <= sizeof(sect_array[pos]))
	    return cur_menu;
    }
