...
set_guild_phase(string short_name, string phase)
{
    ...
    if ((position = member(phase, GUILD_PHASES)) != -1)
...
{
    if (member(master, guilds[short_name][GUILD_MASTERS]) == -1)
...
guild_command(string str)
{
    ...
    if (object_name(previous_object()) != WIZ_CMD_APPRENTICE)
    {
	...
	case 2:
	    guild = lower_case(args[1]);
	    if ((member(guild, names) == -1) || !pointerp(guilds[guild]))
	    {
	...
	case 3:
	    ...
	    if (member(args[2], GUILD_PHASES) == -1)
	    ...
	    default:
		if (member(wname, guilds[guild][GUILD_MASTERS]) == -1)
		{
		...
		while(--index >= 0)
		{
		    if (GUILD_PHASES[index][0 .. (sizeof(args[2]) - 1)] ==
			args[2])
		    {
