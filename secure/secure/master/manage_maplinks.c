...
public int within_box(string wlc, string box)
{
    ...
    if (!sizeof(wlc) || sscanf(wlc, "x%dy%d", xp, yp) != 2)
	return 0;

    if (!sizeof(box) || sscanf(box, "x%d-%dy%d-%d", x, xd, y, yd) != 4)
	return 0;
...
remove_maproom()
{
    ...
    fname = object_name(previous_object());
