...
create()
{
    ...
    load_map = filter(BbpMap, sizeof @ &operator([])(, BBP_BOARD));
    BobMap = ([]);
...
delete_admin(string who)
{
    ...
    else if (member(who, BASE_ADMIN) >= 0)
    {
	MBS->err_args(who);
...
list_admin(int admin)
{
    ...
    for (i = 0 ; i < sz ; i ++)
    {
	if (!admin || member(list[i], BASE_ADMIN) < 0)
	    write(list[i]);
	else
	    write("[" + list[i] + "]");
	if (i < sz - 1)
...
//    data[3] = UC(data[3]);
    ...
    /* String lengths */
    if (sizeof(data[3]) > DESC_LEN)
    {
	MBS->err_args(data[3]);
	return MBM_STR_LONG;
    }
    if (sizeof(data[0]) > NAME_LEN)
    {
	MBS->err_args(data[0]);
	return MBM_STR_LONG;
    }
...
list_boards(int order, int admin, string spec)
{
    ...
    {
	spec = UC(spec);
	if (member(spec, SECURITY->query_domain_list()) >= 0)
	    order = ORDER_DOMAIN;
	else if (sizeof(CategoryMap[spec]))
	    order = ORDER_CAT;
	else
	{
	    MBS->err_args(spec);
...
add_category(string cath, string desc)
{
    ...
    else if (sizeof(cath) > NAME_LEN)
    {
	MBS->err_args(cath);
	return MBM_STR_LONG;
    }
    else if (sizeof(desc) > DESC_LEN)
    {
	MBS->err_args(desc);
	return MBM_STR_LONG;
    }
...
list_categories(int admin)
{
	...
	for (i = 0, sz = sizeof(caths) ; i < sz ; i++)
	{
	    if (admin && sizeof(BASE_CAT[caths[i]]))
...
generate_report(int what)
{
    ...
    blist = filter(blist, &operator(!=)(0) @ sizeof @
		   &operator([])(, BBP_BOARD));
...
query_boards(int order, string spec, string *excl)
{
    ...
    bmap = filter(BbpMap, sizeof @ &operator([])(, 0));
...
find_board(string bspath)
{
    ...
    if ((i = member(bspath, obs->query_board_name())) >= 0)
    {
	BobMap[bspath] = obs[i];
	return obs[i];
    }
...
update_bbmaps()
{
    ...
    entry_map = filter(BbpMap, sizeof @ &operator([])(, 0));
...
mail_notify(int what, mixed list)
{
    ...
    if (member(dom, NO_LORD_DOMAIN) >= 0)
    {
	...
	recipient = sizeof(lord) ? lord : dom;
    }
...
load_all_boards(string *list)
{
	...
	if (tmfunc(BbpMap[list[0]][BBP_LNOTE]) > DTS(WARN_DELAY) &&
	    !sizeof(BASE_CAT[BbpMap[list[0]][BBP_CAT]]))
	{
