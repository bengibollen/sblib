 ...
 */
int     dom_count = 0;      /* The next domain number to be added */
mapping m_domains = ([ ]);   /* The domain mapping */
mapping m_wizards = ([ ]);   /* The wizard mapping */
mapping m_applications = ([ ]); /* The applications mapping */
mapping m_global_read = ([ ]);  /* The global read mapping */
mapping m_teams = ([ ]);        /* The arch team mapping */
...
query_domain_members(string dname)
{
    ...
    return SIMUL_EFUN->secure_var(m_domains[dname][FOB_DOM_MEMBERS]);
}
...
draft_wizard_to_domain(string dname, string wname)
{
    ...
    if (sizeof(m_wizards[wname][FOB_WIZ_DOM]) &&
...
leave_domain()
{
    ...
    if (!sizeof(m_wizards[cmder][FOB_WIZ_DOM]))
    {
...
regret_application(string dname)
{
    ...
    if (member(wname, m_applications[dname]) == -1)
    {
...
list_applications_by_wizard(string wname, int list_self)
{
    ...
    wname = lower_case(wname);
    domains = filter(m_applications, (: filter_applications($1, wname) :));
...
public void bookkeep_exp(string type, int exp)
{
    ...
    if (!sizeof(m_domains[dname]))
    {
        ...
        if ((dname != BACKBONE_UID) &&
            (object_name(giver) != CMD_LIVE_THIEF))
        {
            log_file(LOG_BOOKKEEP_ERR,
...
//      m_domains[dname][FOB_DOM_GXP] += exp;
    ...
    if (should_log)
    {
        log_file(LOG_BOOKKEEP, sprintf("%s %-12s %-7s(%8d) %-1s\n",
            ...
            type, exp, object_name(giver)), 1000000);
    }
#endif
...
query_wiz_list(int rank)
{
    ...
    return filter(m_indices(m_wizards), (: query_wiz_rank($1) == rank :));
}
...
static void do_change_rank(string wname, int rank, string cmder)
{
    ...
    if (rank == WIZ_LORD)
    {
        ...
        if (sizeof(m_domains[dname][FOB_DOM_LORD]))
    ...
    {
        dname = m_wizards[wname][FOB_WIZ_DOM];
        if (sizeof(m_domains[dname][FOB_DOM_STEWARD]))
...
{
    if (member(function_exists("make_wiz", previous_object()),
        WIZ_MAKER) == -1)
        return;
...
set_mentor(string mentor, string student)
{
    ...
    if (sizeof(m_wizards[student][FOB_WIZ_MENTOR]) > 0)
        return 0;

    /* It's impossible to be a mentor and student at the same time. */
    if (sizeof(m_wizards[mentor][FOB_WIZ_MENTOR]) > 0 ||
...
add_student(string mentor, string student)
{
    ...
    if (member(student, m_wizards[mentor][FOB_WIZ_STUDENTS]) >= 0)
        return 0;
...
query_team_member(string team, string member)
{
    ...
    member = lower_case(member);
    return (member(member, m_teams[team]) >= 0);
}
