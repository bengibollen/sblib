...
sanction(string str)
{
    ...
    while(++index < size)
    {
	if (member(parts[index], PERSONAL_SANCTIONS) == -1)
	{
...
domainsanction(string str)
{
    ...
    if (!sizeof(str))
    {
	if (!sizeof(str = query_wiz_dom(name)))
	{
    ...
    while(++index < size)
    {
	if (member(parts[index],
			 (sizeof(path) ? PATH_SANCTIONS :
			  DOMAIN_SANCTIONS)) == -1)
	{
	    write("Sanction \"" + parts[index] +
		  "\" is not possible as domain " +
		  (sizeof(path) ? "path " : "") + "sanction.\n");
	    continue;
	}
