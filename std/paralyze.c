...
stop(string str)
{
    ...
    if (member(query_verb(), CMDPARSE_PARALYZE_ALLOWED) != -1)
    {
        return 0;
    }
...
stat_object()
{
    ...
    if (sizeof(stop_message))
    {
        str += "Stop mess: " + stop_message + "\n";
    }
    if (sizeof(fail_message))
    {
    ...
    if (objectp(stop_object))
    {
        str += "Stop obj:  " + object_name(stop_object) + "\n";
    }
