 ...
 * Prototype.
 */
public void set_coin_type(string str);
...
{
    string str = (sizeof(coin_type) ? (coin_type + " ") : "") + "coin";
    ...
    if (({int}) for_object->query_stat(SS_INT) / 2 > num_heap())
    {
...
public void config_split(int new_num, object orig)
{
    ...
    set_coin_type(({string}) orig->query_coin_type());
}
...
varargs public int move(mixed dest, mixed subloc)
{
	...
	capitalize(({string}) env->query_real_name()) : object_name(env)) : "void");
    str += " -> ";
    str += (interactive(dest) ? capitalize(({string}) dest->query_real_name()) :
	object_name(dest));
