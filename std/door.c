...
int pass_door(string arg)
{
    ...
    if (open_status)
    {
        ...
        */
        if ((int)this_player()->query_prop(CONT_I_HEIGHT) > query_prop(DOOR_I_HEIGHT) * dexh)
        {
        ...
        else if ((int)this_player()->query_prop(CONT_I_HEIGHT) > query_prop(DOOR_I_HEIGHT))
        {
 ...
 */
static int this_door(string arg)
{
    object *objs;
...
int unlock_door(string arg)
{
    ...
    {
        write("Ok.\n");
        say(QCTNAME(this_player()) + " " + check_call(unlock_mess[0]), this_player());
        do_unlock_door("");
...
int lock_procedure(string arg)
{
    ...
    if (key_str && (sizeof(key_ob = FIND_STR_IN_OBJECT(key_str, this_player()))))
    {
...
void do_pick_lock(int skill, int pick)
{
    if (skill > pick)
    {
        ...
        write("You get very satisfied when you hear a soft 'klick' from the lock.\n");
...
int knock_door(string arg)
{
    ...
    if (!open_status)
    {
        ...
        say(QCTNAME(this_player()) + " " + check_call(knock_resp[1]), this_player());
    ...
    else
    {
        ...
        say(QCTNAME(this_player()) + " " + check_call(knock_fail[1]), this_player());
 ...
 */
void set_lock_command(mixed command)
{
    if (!command)
    {
    	lock_commands = ({ });
 ...
 */
void set_unlock_command(mixed command)
{
    if (!command)
    {
    	unlock_commands = ({ });
 ...
 */
string *query_unlock_command() 
{ 
    return unlock_commands; 
}
...
void load_other_door()
{
     ...
     */
    if (!sizeof(other_room) || other_door)
    {
    	return;
    }
    ...
    pos = member(door_id, door_ids);

    if (pos < 0)
    {
        write("Other side of door is not in room after it has been loaded: " + other_room + ".\n");
        remove_door_info(environment(this_object()));
        remove_object();
 ...
 */
static void add_door_info(object dest)
{
    string *door_ids;
    object *doors;
 ...
 */
string standard_open_desc()
{
    string temp_desc;
    temp_desc = "There is an open " + short();

    if (sizeof(pass_commands[0]) <= 2)
    {
...
string standard_knock_fail1()
{
    return "foolishly " + query_verb() + "s on the open " + short() + ".\n";
}
...
string standard_knock_fail2()
{
     ...
     * for an invisible someone knocking on this side of the door.
     */
    return "You hear someone foolishly " + query_verb() + " on the open " + short() + ".\n";
}
